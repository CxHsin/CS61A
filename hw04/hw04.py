...
def balanced(m):
    ...
    """
    "*** YOUR CODE HERE ***"
    total_mass_left = total_mass(end(left(m))) * length(left(m))
    total_mass_right = total_mass(end(right(m))) * length(right(m))
    if total_mass_left != total_mass_right:
        return False
    left_end = end(left(m))
...
def max_path_sum(t):
    ...
    if is_leaf(t):
        ...
        branche_sums = [max_path_sum(b) for b in branches(t)]
    return label(t) + max(branche_sums)
